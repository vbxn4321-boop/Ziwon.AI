...
export interface PsstGeneratorInput {
  ...
  /** 공고 첨부 HWPX 서식 스키마 — 공고 고유 칸 구조 1:1 조립용 */
...
export interface PsstBusinessPlanResult {
  /** 실제 공고 첨부 서식의 각 칸(Field)별로 조립된 상세 본문 */
  formSections?: Array<{
    id: string;
    label: string;
    sectionTitle?: string;
...
4. **공고 맞춤 전략 반영**: 위 공고 AI 분석 데이터 및 서식 칸 지침이 제공된 경우, 배점 비중이 높은 항목 및 고유 서식 항목(formSections)을 충실히 채우세요.
  ...
  "formSections": [
    {
      ...
      "guidance": "작성지침",
      "type": "NARRATIVE",
      "content": "대화 및 입력 내용을 근거로 해당 서식 칸의 지침에 맞게 작성된 2~4단락의 고밀도 본문"
    }
  ],
  "overview": {
...
`.trim();
  ...
  for (const modelName of candidateModels) {
    try {
      ...
      // 모델이 반환한 항목에 원본 서식의 위치 메타데이터를 다시 결합한다.
      // 일부 모델이 선택 항목을 누락해도 모든 채울 칸이 결과물에 남도록 보정한다.
      if (input.formSchema?.fields?.length) {
        const generated = new Map((parsed.formSections || []).map((section) => [section.id, section]));
        parsed.formSections = input.formSchema.fields.map((field) => {
          const section = generated.get(field.id);
          return {
            id: field.id,
            label: field.label,
